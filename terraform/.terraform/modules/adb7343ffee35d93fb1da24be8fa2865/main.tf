...
resource "digitalocean_droplet" "hackerlite-droplet" {
  ...
  provisioner "file" {
    ...
    destination = "/etc/nginx/nginx.conf" // copy to different location then overwrite
  }
  ...
  provisioner "remote-exec" {
    ...
    # Install Certbot - Let's Encrypt
      ...
      "sudo add-apt-repository -y -q ppa:certbot/certbot",
      "sudo apt-get update",
      "sudo apt-get install -y -q python-certbot-nginx",
